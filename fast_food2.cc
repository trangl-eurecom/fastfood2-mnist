...
REGISTER_OP("FastFood2")
    ...
    .Input("in5: float") // diagonal matrix S: [Dfeatures, 1] tensor
...
class FastFood2Op : public OpKernel {
    public:
    explicit FastFood2Op(OpKernelConstruction* context) : OpKernel(context) {}
    int batch_size = 0;
    int Dfeatures = 0;
    ...
    // Output:
    //  + y: is a permutation of x created by pi
    ...
    //  + S_tensor: [Dfeatures, 1]: random scaling matrix, S[i] = s[i] * ..., in paper Fastfood
    // Output:
...
REGISTER_KERNEL_BUILDER(Name("FastFood2").Device(DEVICE_CPU), FastFood2Op);
