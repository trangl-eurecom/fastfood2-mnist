...
class DgpRff(object):
    def __init__(self, likelihood_fun, num_examples, d_raw_in, d_out, n_layers, n_rff, df, kernel_type, kernel_arccosine_degree, is_ard, feed_forward, q_Omega_fixed, theta_fixed, learn_Omega):
        ...
        self.fastfood_module = tf.load_op_library('fast_food2/fast_food2.so')
    ...
    def init_random_vector(self):
        ...
        for i in range(self.nl):
            ...
            # At first, S will be sampled from N(0, 1). After code can work without bug, I will correct the fast food, S * ||G||
    ...
    def zero_padding_32_32(self, x_images):
        ...
        x_images_zero_padding = tf.concat([zero_padding_row, x_images], 1)
        x_images_zero_padding = tf.concat([x_images_zero_padding, zero_padding_row], 1)
        x_images_zero_padding = tf.concat([zero_padding_column, x_images_zero_padding], 2)
        x_images_zero_padding = tf.concat([x_images_zero_padding, zero_padding_column], 2)
